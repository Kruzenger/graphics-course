...
float sdf(in vec3 p,in mat3 m){
    ...
    return min(dSphere(q),dSphere2(q));
}

vec3 trace(in vec3 st,in vec3 dir,out bool hit,in mat3 m){
    vec3 p=st;
    hit=false;
...
void main()
{
    ...
    vec2 mouse=vec2(params_t.mouse_x/iResolution.x-.5,params_t.mouse_y/iResolution.y-.5);
    mat3 m=rotateX(6.*mouse.y)*rotateY(6.*mouse.x);
    ...
    vec3 p=trace(eye,dir2,hit,m);
    vec3 color=vec3(0.);
    
    if(hit){
        vec3 ld=normalize(light-p);
        vec3 ed=normalize(eye-p);
        vec3 norm=generateNormal(p,m);
    ...
    float threeNPlusOne=ThreeNPlusOne();
    threeNPlusOne*=.0001;
    threeNPlusOne*=.0001;
    color+=vec3(threeNPlusOne);
